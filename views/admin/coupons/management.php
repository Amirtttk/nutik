                                ...
                                <div class="tab-pane fade show active" id="kt_tab_pane_7_3" role="tabpanel">
                                    <table class="table table-bordered table-hover table-checkable" id="manager"
                                        ...
                                        <tbody>
                                            ...
                                            if ($getAllCoupon) {
                                                foreach ($getAllCoupon as $key => $AllCoupon) {
                                                    $date = jdate('Y/m/d ', (int)$AllCoupon['start_date']);;
                                                    $date2 = jdate('Y/m/d ', (int)$AllCoupon['end_date']);
                                                    ?>
                                                    <tr id="deleteFaq<?php echo $AllCoupon['id']?>">
                                                        <td><?= $keyNumber++ ?></td>
                                                        <td><?= $AllCoupon['code'] ?></td>
                                                        <td><?= number_format($AllCoupon['discount_value']) ?></td>
                                                        <td>
                                                            <?php
                                                            if ($AllCoupon['discount_type'] == 'percent') {
                                                                ?>
                                                                <span
                                                                        class="label label-lg font-weight-bold label-light-success label-inline">درصدی</span>
                                                                <?php
                                                            } else {
                                                                ?>
                                                                <span
                                                                        class="label label-lg font-weight-bold label-light-warning label-inline">قیمت</span>
                                                                <?php
                                                            }
                                                            ?>
                                                        </td>
                                                        <td><?= $date ?></td>
                                                        <td><?= $date2 ?></td>
                                                        <td><?= $AllCoupon['usage_limit'] ?></td>
                                                        <td><?= number_format($AllCoupon['min_purchase']) ?></td>
                                                        <td>
                                                            <?php
                                                            if($AllCoupon['once_per_user'] == '1'){
                                                            echo 'هر نفر یک بار';
                                                            }else{
                                                                echo 'تعداد نامحدود';
                                                            }
                                                        ...
                                                        <td id="statusCopens<?= $AllCoupon['id'] ?>">
                                                            <?php
                                                            if ($AllCoupon['status'] == 2) {
                                                                ?>
                                                                <span class="switch switch-icon">
                                                                    <label>
                                                                        <input id="changeStatusInput<?= $AllCoupon['id'] ?>"
                                                                            onclick="statusCopens(<?= $AllCoupon['id'] ?>, 1)"
                                                                ...
                                                                <span class="switch switch-icon">
                                                                    <label>
                                                                        <input
                                                                            id="changeStatusInput<?= $AllCoupon['id'] ?>"
                                                                            onclick="statusCopens(<?= $AllCoupon['id'] ?>, 2)"
                                                        ...
                                                        <td>
                                                            <a href="<?= url("/admin/coupons/update?id={$AllCoupon['id']}") ?>"
