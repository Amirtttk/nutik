...
if (!empty($_FILES['image']['size'])) {
    ...
    // مسیر فیزیکی ذخیره
    $uploadPath = PATH_UPLOADS_DIR . 'images/advertising_banner/' . $new_name;
    if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadPath)) {
        $getOneBlog = getOneAdvertising(POST('id'));
        // ✅ فقط نام فایل ذخیره شود
        $fields = [
            'image_name' => $new_name,
        ];

        $updatePhoto = updateRecordToDatabase('advertising_banner', $fields, POST('id'), 'id');
        if ($updatePhoto) {
            // حذف تصویر قبلی
            if (!empty($getOneBlog['image_name'])) {
                $oldPath = PATH_UPLOADS_DIR . 'images/advertising_banner/' . $getOneBlog['image_name'];
            ...
            responseJson([
                ...
                'src' => '/images/advertising_banner/' . $new_name,
                'oldImage' => 'yes'
            ]);
        } else {
            responseJson([
