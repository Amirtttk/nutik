...
if (!empty($_FILES['image']['size'])) {
    ...
    // مسیر فیزیکی ذخیره
    $uploadPath = PATH_UPLOADS_DIR . 'images/aboutus/' . $new_name;
    if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadPath)) {
        $getOneBlog = getAllaAboutUs();
        // ✅ فقط نام فایل ذخیره شود
        $fields = [
            'image_name' => $new_name,
        ];
        $updatePhoto = updateRecordToDatabase('aboutus', $fields, POST('id'), 'id');
        if ($updatePhoto) {
            // حذف تصویر قبلی
            if (!empty($getOneBlog['image_name'])) {
                $oldPath = PATH_UPLOADS_DIR . 'images/aboutus/' . $getOneBlog['image_name'];
            ...
            responseJson([
                'text' => 'تصویر درباره ما با موفقیت ویرایش شد',
                'type' => 'success',
                'status' => 200,
                // ✅ فقط URL نسبی برگردان
                'src' => '/images/aboutus/' . $new_name,
                'oldImage' => 'yes'
            ]);
        } else {
            responseJson([
