...
if (!empty($_FILES['image']['size'])) {
    ...
    // مسیر فیزیکی ذخیره
    $uploadPath = PATH_UPLOADS_DIR . 'images/brand/' . $new_name;
    if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadPath)) {
        $getOneBlog = getOneBrand(POST('id'));
        // ✅ فقط نام فایل ذخیره شود
        $fields = [
            'image_name' => $new_name,
        ];

        $updatePhoto = updateRecordToDatabase('brand', $fields, POST('id'), 'id');
        if ($updatePhoto) {
            // حذف تصویر قبلی
            if (!empty($getOneBlog['image_name'])) {
                $oldPath = PATH_UPLOADS_DIR . 'images/brand/' . $getOneBlog['image_name'];
