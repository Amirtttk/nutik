...
if (!empty($_FILES['image']['size'])) {
    ...
    // مسیر فیزیکی ذخیره
    $uploadPath = PATH_UPLOADS_DIR . 'images/blog_categories/' . $new_name;
    if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadPath)) {
        $getOneBlog = getOneBlogCategories(POST('id'));
        // ✅ فقط نام فایل ذخیره شود
        $fields = [
            'image_name' => $new_name,
        ];

        $updatePhoto = updateRecordToDatabase('blog_categories', $fields, POST('id'), 'id');
        if ($updatePhoto) {
            // حذف تصویر قبلی
            if (!empty($getOneBlog['image_name'])) {
                $oldPath = PATH_UPLOADS_DIR . 'images/blog_categories/' . $getOneBlog['image_name'];
            ...
            responseJson([
                ...
                'type' => 'success',
                'status' => 200,
                'src' => '/images/blog_categories/' . $new_name,
                'oldImage' => 'yes'
            ]);
        } else {
            responseJson([
